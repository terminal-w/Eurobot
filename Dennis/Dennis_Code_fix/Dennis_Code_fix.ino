...
float WheelBaseDiameter = 26.5;                               // Distance between the two motor driven wheels in CM
...
void Motion(int Speed, float Distance, char Direction, int Turn){
  if (Direction = 's'){                                               //forward direction
    ...
    Distance1 = Distance;
    Distance2 = Distance;
      if (Encoder(2) <= Distance1 && Encoder(2) <= Distance2){          // If statement to check the status of the traveled distance    
      Transmit(ACCELERATION, Acceleration);                                // Sets the acceleration to register 1 (6.375s)
      Transmit(SPEED, DualSpeed);                                // Sets a combined motor speed value    
      Motion(Speed, Distance, Direction, Turn);                                                          
    }
    BotReset();                                                           //Resets the encoders and stops the motor
  }
